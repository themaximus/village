...
public class MovePlayer : MonoBehaviour, ISaveable
{
    ...
    void Awake()
    {
        ...
        {
            // Если да (например, мы вернулись на главную сцену),
            // то уничтожаем этого "нового" игрока, оставляя старого.
            Destroy(gameObject);
            return; // Прерываем выполнение Awake
        }
        else
        {
            // Если это первый игрок, делаем его "вечным"
            instance = this;
    ...
    /// <summary>
    /// "Фотографирует" текущую позицию игрока.
    /// </summary>
    ...
    /// <summary>
    /// Восстанавливает позицию игрока из "фотографии".
    /// </summary>
